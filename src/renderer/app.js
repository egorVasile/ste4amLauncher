...
(function () {
  ...
  function buildNews() {
    const mk = (n, i) => {
      const title = escapeHtml(Array.isArray(n) ? n[0] : n.title);
      const desc = escapeHtml(Array.isArray(n) ? (n[1] || '') : (n.desc || ''));
      const date = Array.isArray(n) ? n[2] : n.date;
      const icon = Array.isArray(n) && n[3] ? n[3] : NEWS_ICONS[i % NEWS_ICONS.length];
      const link = Array.isArray(n) ? '' : (n.link || '');
      return `
    ...
    const wire = (box, arr) => {
      ...
      box.querySelectorAll('.news-item').forEach(it => {
        const u = it.getAttribute('data-link');
        if (!u) return;
        it.style.cursor = 'pointer';
        it.addEventListener('click', () => api.invoke('shell:open-url', u));
      });
    };
    const list = $('#newsList');
  ...
  function buildSettingsPanel() {
    ...
    if (!panel) return;
    panel.innerHTML = `
      <div class="set-row"><div><div class="s-label">ТЕМА</div><div class="s-desc">Официальная светлая или тёмная (как в лаунчере Mojang)</div></div><div class="theme-seg" id="themeSeg"><button class="b-type-btn t-opt${settingsCache.theme === 'light' ? ' sel' : ''}" data-t="light">СВЕТЛАЯ</button><button class="b-type-btn t-opt${settingsCache.theme === 'dark' ? ' sel' : ''}" data-t="dark">ТЁМНАЯ</button><button class="b-type-btn t-opt${settingsCache.theme === 'system' ? ' sel' : ''}" data-t="system">СИСТЕМА</button></div></div>
      <div class="set-row"><div><div class="s-label">ИМЯ (OFFLINE)</div><div class="s-desc">Ник для оффлайн-режима</div></div><input type="text" id="nickInput" value="${escapeHtml(settingsCache.username)}" style="background:var(--mc-grey-5);border:2px solid var(--mc-off-black);color:var(--mc-off-white);padding:7px 10px;font-family:var(--mc-font);font-size:13px;width:180px;outline:none"/></div>
      <div class="set-row"><div><div class="s-label">ПАМЯТЬ (RAM)</div><div class="s-desc">Выделенная память для игры (макс. ${settingsCache.totalRam} GB — реальная)</div></div><input type="range" id="ram2" min="1" max="${settingsCache.totalRam}" step="1" value="${safeRam(settingsCache.ram)}"/><div class="s-value" id="ram2v">${safeRam(settingsCache.ram)} GB</div></div>
      ...
      <div class="set-row"><div><div class="s-label">ЗЕРКАЛО</div><div class="s-desc">auto = Mojang, при ошибке BMCLAPI</div></div><div class="dropdown" style="padding:6px 10px;font-size:11px" id="mirrorDD">&#9662; ${settingsCache.mirror}</div></div>
      <div class="set-row"><div><div class="s-label">JAVA</div><div class="s-desc">Найденная Java при запуске</div></div><button class="dropdown" style="padding:6px 10px;font-size:11px" id="javaBtn">НАЙТИ JAVA</button></div>
      ...
      <div class="set-row"><div><div class="s-label">ЛАУНЧЕР ПОСЛЕ ЗАПУСКА</div><div class="s-desc">Как вести себя после старта игры</div></div><div class="theme-seg" id="lmSeg"><button class="b-type-btn t-opt${settingsCache.launcherMode === 'keep' ? ' sel' : ''}" data-lm="keep">ОСТАВИТЬ</button><button class="b-type-btn t-opt${settingsCache.launcherMode === 'minimize' ? ' sel' : ''}" data-lm="minimize">СВЕРНУТЬ</button><button class="b-type-btn t-opt${settingsCache.launcherMode === 'close' ? ' sel' : ''}" data-lm="close">ЗАКРЫТЬ</button></div></div>
      <div class="set-row"><div><div class="s-label">КАТАЛОГ ИГРЫ</div><div class="s-desc">Открыть папку .st4amlauncher/game</div></div><button class="dropdown" style="padding:6px 10px;font-size:11px" id="openDirBtn">ОТКРЫТЬ</button></div>
      <div class="set-row"><div><div class="s-label">ПАПКИ ЛАУНЧЕРА</div><div class="s-desc">Открыть корневую папку или папку модов</div></div><div style="display:flex;gap:8px"><button class="dropdown" style="padding:6px 10px;font-size:11px;flex:1" id="openRootBtn">КОРНЕВАЯ</button><button class="dropdown" style="padding:6px 10px;font-size:11px;flex:1" id="openModsBtn">MODS</button></div></div>
      <div class="set-row" id="expRow"><div><div class="s-label">ЭКСПЕРИМЕНТАЛЬНО</div><div class="s-desc" style="color:var(--mc-green-2)">Сборки модов, каталог Modrinth, многое другое</div></div><div class="switch ${settingsCache.experimental ? 'on' : ''}" id="expSw"></div></div>
      <div class="set-row"><div><div class="s-label">СБРОСИТЬ</div><div class="s-desc">Вернуть значения по умолчанию</div></div><button class="dropdown" style="padding:6px 10px;font-size:11px" id="resetBtn">СБРОС</button></div>`;
